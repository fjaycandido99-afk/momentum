  ...
  try {
    ...
    switch (type) {
      ...
      case 'coach_checkin':
        ...
        return NextResponse.json({ success: true, type: 'coach_checkins' })

      case 'coach_accountability':
        await sendCoachAccountability()
        return NextResponse.json({ success: true, type: 'coach_accountability' })
      ...
      // The morning heads-up, before the day is spent.
      ...
      case 'era_wake':
        ...
        return NextResponse.json({ success: true, type: 'era_wake' })

      case 'era_complete':
        await sendEraCompletions()
        return NextResponse.json({ success: true, type: 'era_complete' })

      case 'era_comeback':
        await sendEraComebacks()
        return NextResponse.json({ success: true, type: 'era_comeback' })
