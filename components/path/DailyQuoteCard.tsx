    ...
    <div className="card-path p-5">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-medium text-white/90">Daily Quote</h3>
        </div>
        <ShareButton
          title="Daily Quote"
      ...
      <blockquote className={`pl-3 border-l-2 ${accent}`}>
        <p className="text-[14px] text-white/80 italic leading-relaxed">&ldquo;{quote.text}&rdquo;</p>
        <footer className="mt-2 text-xs text-white/45">— {quote.author}</footer>
      </blockquote>

      {quote.category && (
        <div className="mt-3 flex">
          <span className="px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.08] text-[10px] text-white/35 uppercase tracking-wider">
            {quote.category}
          </span>
        </div>
      )}
