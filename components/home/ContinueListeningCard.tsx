      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-4 p-4">
          <button
            ...
            className="shrink-0 w-12 h-12 rounded-full bg-white/15 border border-white/20 flex items-center justify-center hover:bg-white/25 transition-colors"
          >
            <Play className="w-5 h-5 text-white ml-0.5" fill="white" />
          </button>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-white/60 font-medium uppercase tracking-wide mb-0.5">Continue listening</p>
            <p className="text-sm text-white/95 line-clamp-1 font-medium">{video.title}</p>
            <p className="text-xs text-white/50 mt-0.5">{formatDuration(remaining)} remaining</p>
            {/* Progress bar */}
