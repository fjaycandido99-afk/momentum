...
describe('cleanPlan', () => {
  it('keeps their lines, trimmed', () => {
    ...
    expect(plan).toEqual({ push: ['Lat pulldown', 'Squat'] })
  })
  ...
  it('drops blank lines and empty slots', () => {
    expect(cleanPlan({ push: ['', '   '], pull: ['Rows'] }, ppl)).toEqual({ pull: ['Rows'] })
  })
  ...
  it('caps how much can be written', () => {
    ...
    expect(plan.push).toHaveLength(PLAN_MAX_ITEMS)
    const longPlan = cleanPlan({ push: [long] }, ppl)
    expect(longPlan.push[0]).toHaveLength(PLAN_MAX_ITEM_LENGTH)
  })
  ...
  it('takes a book for a reading practice', () => {
    expect(cleanPlan({ any: ['Rich Dad Poor Dad'] }, everyDay)).toEqual({ any: ['Rich Dad Poor Dad'] })
  })
})

describe('parsePlan and planFor', () => {
  it('reads a stored plan back', () => {
    const plan = parsePlan({ push: ['Bench'], junk: 'no' })
    expect(plan).toEqual({ push: ['Bench'] })
  })
  ...
  it('finds today’s lines, and copes with neither existing', () => {
    const plan = { pull: ['Rows'] }
    expect(planFor(plan, { key: 'pull', label: 'Pull' })).toEqual(['Rows'])
    expect(planFor(plan, { key: 'push', label: 'Push' })).toEqual([])
    expect(planFor(plan, null)).toEqual([])
    expect(planFor(null, { key: 'pull', label: 'Pull' })).toEqual([])
  })

  it('knows whether anything is written down', () => {
    expect(hasPlan({ pull: ['Rows'] })).toBe(true)
    expect(hasPlan({ pull: [] })).toBe(false)
