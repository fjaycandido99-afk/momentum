...
/**
 * NOTE ON `backgroundPool`: nothing renders it.
 *
 * The animated per-mindset backgrounds it names (falling-sand, hourglass,
 * columns…) were deleted along with DailyBackground and MoodTintOverlay. The
 * backgrounds that DO exist are per music genre, from /api/backgrounds, and
 * have nothing to do with a mindset. The pool is kept in case those visuals
 * come back — but it is not a feature, so nothing may advertise it. Eight
 * lines of "<X> background visuals" sat on the mindset commit screen for
 * months because this array looked live. See the gate test in
 * __tests__/configs.test.ts.
 */
...
export const MINDSET_CONFIGS: Record<MindsetId, MindsetConfig> = {
  ...
  scholar: {
    id: 'scholar',
    name: 'Scholar',
    // "includes astrology" was here, and it was a promise the app could not
    // keep: astrology was deleted. The picker is the first screen anybody
    // reads, so a feature named there had better exist.
    subtitle: 'Explore mind, myth & cosmos through archetype',
